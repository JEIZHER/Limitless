	...
	data_sec &&
		...
		data_sec?.map((cripto) => {
			...
			if(allChildren){
					...
					color = Number(cripto.c) > Number(cripto.o) ? '#0ECB81' : Number(cripto.c) === Number(cripto.o) ?'white':'#F6465D'
					...
					ctx.stroke()
						...
						allChildren[1].children[0].children[2].textContent = cripto.h> 1 ? Number(cripto.h).toFixed(2) : Number(cripto.h)
						allChildren[1].children[0].children[3].textContent = cripto.l> 1 ? Number(cripto.l).toFixed(2) : Number(cripto.l)
					}
